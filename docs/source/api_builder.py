...
def create_dir(parents, package, opts):
    """Create a directory for a package.

    """
    path = os.path.join(os.path.join(opts.destdir, *parents.split('.')),
                        package)
    if not os.path.isdir(path):

        if opts.dryrun:
            print('Would create folder %s.' % path)
            return

        else:
            print('Creating folder for %s package' % package)
            os.makedirs(path)
...
def create_package_file(root, master_package, subroot, py_files, opts, subs):
    ...
    if submods:
        ...
        if opts.separatemodules:
            ...
            for submod in submods:
                ...
                filetext += format_directive(makename(subroot, submod),
                                             master_package)
                modpath = os.path.join(os.path.join(*subroot.split('.')),
                                       submod)
                write_file(modpath, filetext, opts)
        else:
            for submod in submods:
                modfile = makename(master_package, makename(subroot, submod))
                if not opts.noheadings:
    ...
    if not opts.tree_struct:
        write_file(makename(master_package, subroot), text, opts)
    else:
        write_file(os.path.join(os.path.join(*subroot.split('.')),
                                'index'), text, opts)
...
Note: By default this script will not overwrite already created files.""")
    ...
    parser.add_argument('exclude_paths', nargs='*')
    ...
    parser.add_argument('-e', '--separate', action='store_true',
                        ...
                        help='Put documentation for each module on its own '
                             'page')
    parser.add_argument('--tree-struct', action='store_true',
                        help='Should subfolders be created for subpackages')
    ...
    rootpath, excludes = args.path, args.exclude_paths
    if not args.destdir:
        parser.error('An output directory is required.')
    if args.header is None:
        args.header = path.abspath(rootpath).split(path.sep)[-1]
